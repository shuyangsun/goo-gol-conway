...
pub trait BoardStateManager<'data, 'dref, T, CI, I1, I2, I3>: Send + Sync
where
    'data: 'dref,
    T: 'data + Send + Sync,
    CI: Send + Sync,
...
pub trait Board<'data, 'dref, T, CI, I1, I2, I3, I4>
where
    'data: 'dref,
    T: 'data + CellState,
    CI: CellIndex,
    ...
    I2: Iterator<Item = IndexedDataOwned<CI, T>>,
    I3: ParallelIterator<Item = IndexedDataOwned<CI, T>>,
    I4: ParallelIterator<Item = CI>,
{
    fn state_manager(&self) -> &dyn BoardStateManager<'data, 'dref, T, CI, I1, I2, I3>;
    fn state_manager_mut(&mut self) -> &mut dyn BoardStateManager<'data, 'dref, T, CI, I1, I2, I3>;
    fn space_manager(&self) -> &dyn BoardSpaceManager<CI, I1, I4>;
    fn strategy_manager(
        &self,
    ...
    fn advance(&mut self) {
        ...
        let next_states = self.space_manager().indices_par_iter().map(|idx| {
            ...
            let neighbors: Vec<IndexedDataRef<'dref, CI, T>> = space
                ...
                .collect();
            strat
                .get_strategy_at_index(idx)
                .next_state(idx, cur_state, neighbors.into_iter())
        });

        // TODO: Drawing and callbacks.

        self.state_manager_mut()
            .update_cell_states_from_par_iter(next_states.par_iter());
    }
}
